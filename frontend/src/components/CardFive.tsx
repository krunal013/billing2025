...
const CardFive: React.FC<CardFiveProps> = ({ totalAmount }) => {
  const [prevAmount, setPrevAmount] = useState(totalAmount);
  const [digits, setDigits] = useState<string[]>(
    totalAmount.toFixed(2).split(''),
  );

  useEffect(() => {
    if (totalAmount !== prevAmount) {
      const newDigits = totalAmount.toFixed(2).split('');
      setTimeout(() => {
        setDigits(newDigits);
      }, 100); // Small delay to simulate rolling effect
                ...
                <div key={index} className="relative w-4 h-8 overflow-hidden">
                  <AnimatePresence mode="popLayout">
                    <motion.span
                      key={`${digit}-${index}`}
                      initial={{
                        y: prevAmount < totalAmount ? '100%' : '-100%',
                        opacity: 0,
                      }}
