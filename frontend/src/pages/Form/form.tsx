...
function Form() {
  ...
  const predefinedWorks: PredefinedWork[] = [
    { description: "Drafting Of Notice & Director Report and filling form AOC-4 & form MGT-7A for the year ended", amount: 500 },
    { description: "Form AOC-4 vide SRN No.", amount: 1000 },
    { description: "work-3", amount: 1500 },
  ];
  
  const predefinedWorks2: PredefinedWork2[] = [
    { description: "krunal", amount: 500 },
    { description: "Form AOC-4 vide SRN No.", amount: 1000 },
    { description: "work-3", amount: 1500 },
  ];
  ...
  useEffect(() => {
    ...
    axios.get(`${baseUrl}/api/invoice-number`)
      .then(response => {
        setFormData((prevState) => ({
          ...prevState,
  ...
  const handleCompanyChange = (companyId: string) => {
    ...
    if (companyId) {
      axios
        ...
        .then((response) => {
          const { company, dueAmount, totalPayableAmount, totalInvoices } = response.data;
          ...
          axios
            ...
            .catch((error) => console.error("Error fetching next invoice number:", error));
        })
  ...
  const handleSubParticularChange = (particularIndex:number, subIndex:number, field:keyof SubParticular, value:string) => {
    const updatedParticulars = [...formData.particulars];
    updatedParticulars[particularIndex].subParticulars[subIndex][field] = value;
  ...
  const handleDropdownChange = (particularIndex: number, selectedWork: string) => {
    const updatedParticulars = [...formData.particulars];

    const work = (particularIndex === 0 ? predefinedWorks : predefinedWorks2)
  ...
  const handlePDFDownload = async () => {
    if ( !selectedCompanyId || !formData.invoiceNo ||!formData.invoiceid || !formData.invoiceDate) {
    ...
    const transformedData = {
      ...
      ReimbursementExpenses_total: parseFloat(formData.particulars[1]?.total || "0"),
      TotalAmount: parseFloat(formData.total || "0"),
    };
  
    try {
      const response = await axios.post(`${baseUrl}/api/add_invoice`, transformedData, {
      ...
      // link.download = `Invoice_${formData.invoiceid}.pdf`;
      // link.click();
  
      // Reset the form after successful download
      setFormData(initialFormState);
      setSelectedCompanyId(""); // Reset selected company
  ...
  return (
    ...
    <div className="overflow-y-auto h-[calc(100vh-100px)] mt-4 hide-scrollbar" style={{
    scrollbarWidth: "none", // Firefox
    msOverflowStyle: "none", // Internet Explorer 10+
  }}>
      {/* Unpaid Balance Notification */}
      {selectedCompanyDetails && selectedCompanyDetails.dueAmount > 0 && (
        <div
          ...
          <div className="flex">
            ...
            <div>
              <p className="font-bold">
                {selectedCompanyDetails.companyName} currently has an unpaid balance of ₹
                {selectedCompanyDetails.dueAmount}.
              </p>
              <p className="text-sm">
          ...
          >
            {showSummary ? (
              ...
              <div className="flex gap-2 items-center p-3">
                ...
                <div className="leading-3 tracking-wider">Preview</div>
              </div>
            )}
          </button>
              ...
              {/* Company Field */}
              <div>
                <label htmlFor="company" className="block text-[15px] font-semibold mb-2 tracking-wider">
                  Company
                </label>
                <select
      ...
      {companies && companies.length > 0 ? (
        ...
        <option disabled>No Companies Available</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="invoiceNo" className="block text-[15px] font-semibold mb-2 tracking-wider mb-1">
                    Invoice Number
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor="invoiceDate" className="block text-[15px] font-semibold mb-2 tracking-wider mb-1">
                    Invoice Date
                  </label>
                  <input
                    type="date"
            ...
            <div className="mt-9 space-y-9">
              {formData.particulars.map((particular, index) => (
                <div key={index} className=" border border-[#3c4fe01d] bg-white rounded-md shadow-lg">
                  ...
                  <div className="p-4 py-5">
                    ...
                    {(index === 0 ? predefinedWorks : predefinedWorks2).map((work, i) => (
                  ...
                  <div className="space-y-3 mt-4 ">
                    {particular.subParticulars.map((subParticular, subIndex) => (
                      <div key={subIndex} className="flex gap-4 items-start relative">
                        ...
                        <div className="group relative w-full">
                          <textarea
                            ...
                            onChange={(e) =>
                              handleSubParticularChange(index, subIndex, "description", e.target.value)
                            }
                            className="p-3 w-full text-sm bg-[#edf6ff] tracking-wider text-black-2 border border-[#e6e9ee] rounded-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                          />
                          ...
                          <button
                            type="button"
                            onClick={() => removeSubParticular(index, subIndex)}
                          ...
                          >
                            <div className="relative group">
                              <MdDelete />
                              {/* <span className="absolute right-full top-1/2 transform -translate-y-1/2 ml-2 px-2 py-1 text-xs text-white bg-black rounded shadow opacity-0 group-hover:opacity-100">
      ...
      {/* Amount Field with Tooltip */}
                                <div className="group flex items-center relative bg-[#edf6ff] border rounded-l-md border-[#e6e9ee] rounded-sm">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleSubParticularChange(index, subIndex, "amount", e.target.value)
                                    }
                                  ...
                                  >
                                    <div className="relative group">
                                      <MdDelete />
                                      {/* <span className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 px-2 py-1 text-xs text-white bg-black rounded shadow opacity-0 group-hover:opacity-100">
                                        Delete Amount
                                      </span> */}
                                    </div>
                                  </button>
                  ...
                  <div className="flex justify-between p-4 -mt-3">
                    <button
                      ...
                      className="h-10 w-10 -mb-2 border-2 flex items-center justify-center  border-[#3c50e0] text-[#3c50e0] rounded-full shadow-md hover:bg-[#3c50e0] hover:text-white hover:border-[#ffffff] transition duration-300 group"
                    >
                      <FaPlus className="" />
                    </button>

                  <p className="text-right tracking-wider text-lg font-semibold mt-4">Subtotal: ₹{particular.total}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-12 mb-7 flex items-center justify-between text-lg font-bold">
              ...
              {/* Right-Aligned Total */}
              <p className="text-right">Total Amount: ₹{formData.total}</p>
            </div>
...
{showSummary && (
    ...
    {/* Scrollable Form Content */}
        <div
         className="overflow-y-auto h-[calc(100vh-100px)] mt-4 hide-scrollbar" style={{
          scrollbarWidth: "none", // Firefox
          msOverflowStyle: "none", // Internet Explorer 10+
          }}
        >
    ...
    {selectedCompanyDetails ? (
      <div className="p-6  bg-gray-50 rounded-lg shadow-md overflow-y-auto h-[calc(100vh-100px)] mt-4 hide-scrollbar" style={{
        scrollbarWidth: "none", // Firefox
        msOverflowStyle: "none", // Internet Explorer 10+
        }}>
        {/* Company Header */}
        ...
        {/* <p className="text-red-600 font-medium mb-4">
          Due: ₹{selectedCompanyDetails?.dueAmount || 0}
        </p> */}
  <br />
        {/* Company Details in Grid Layout */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-black mb-6">
          <p>
            <span className="font-medium">CIN:</span> {selectedCompanyDetails.cinNumber}
          </p>
          <p>
            <span className="font-medium">GST:</span> {selectedCompanyDetails.gstNumber}
          </p>
          <p>
            <span className="font-medium">Contact:</span> {selectedCompanyDetails.contactNumber}
          </p>
          <p>
            <span className="font-medium">Address:</span> {selectedCompanyDetails.address}
          </p>
        </div>

        {/* Invoice Summary */}
        <div>
          <h3 className="text-xl font-semibold text-black mb-4">Invoice Summary</h3>
          {/* <hr className="border-[#001ae23e] mb-4" /> */}
          <div className="space-y-4">
            {formData.particulars.map((particular, index) => (
              <div key={index} className="bg-[#edf6ff] p-4 rounded-lg border-[#daecff] shadow-sm">
                ...
                <ul className="mt-2 ml-4 space-y-2 text-gray-700">
                  {particular.subParticulars.map((subParticular, subIndex) => (
                    ...
                    >
                      ...
                      <span>₹{subParticular.amount}</span>
                    </li>
                  ))}
                </ul>
                <p className="text-right font-medium text-gray-800 mt-2">
                  Subtotal: ₹{particular.total}
                </p>
              </div>
            ))}
          </div>
          <div className="text-right font-bold text-xl mt-6 text-gray-900">
      ...
      <div className="p-6 rounded-lg shadow-md text-center">
        <h3 className="text-xl font-bold text-gray-800 mb-2">No Company Selected</h3>
        <p className="text-gray-600">Please select a company to view its details.</p>
      </div>
    )}
  </div>
</div>
