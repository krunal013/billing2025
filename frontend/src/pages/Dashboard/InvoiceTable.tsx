...
const InvoiceTable: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const invoiceToDelete = invoices.find((invoice) => invoice._id === selectedInvoiceForDelete);
      if (!invoiceToDelete) return;
  
      await axios.delete(`${baseUrl}/api/invoices/${selectedInvoiceForDelete}`);
  
      setInvoices((prevInvoices) =>
        prevInvoices.filter((invoice) => invoice._id !== selectedInvoiceForDelete)
      );
      setFilteredInvoices((prevInvoices) =>
        prevInvoices.filter((invoice) => invoice._id !== selectedInvoiceForDelete)
      );
  
      toast.success(`Invoice from ${invoiceToDelete.company.companyName} has been deleted!`, {
  ...
  const handleCheckboxChange = async (invoiceId: string, checked: boolean) => {
    ...
    const currentInvoice = invoices.find((invoice) => invoice._id === invoiceId);
    ...
    const updatedData = {
      isPaid: checked,
      unpaid_ProfessionalFees: checked ? currentInvoice.ProfessionalFees_total : 0,
      unpaid_ReimbursementExpenses: checked ? currentInvoice.ReimbursementExpenses_total : 0,
    };
  ...
  useEffect(() => {
    ...
    if (companyFilter) {
      filtered = filtered.filter((invoice) =>
        invoice.company.companyName.toLowerCase().includes(companyFilter.toLowerCase())
      );
    }
  ...
  const InvoiceModal: React.FC<{ invoice: Invoices | null; onClose: () => void }> = ({
    invoice,
    onClose,
  }) => {
    ...
    const [editableInvoice, setEditableInvoice] = useState<Invoices | null>(null);
    ...
    const handleUpdate = async () => {
      ...
      try {
        await axios.put(`${baseUrl}/api/update_invoice/${invoice._id}`, editableInvoice);
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-1/2 text-black overflow-y-auto">
          ...
          <div className="modal-body space-y-4">
            ...
            {isEditMode ? (
              ...
              <p><strong>Invoice Number:</strong> {editableInvoice?.invoice_number}</p>
            )}
            ...
            {isEditMode ? (
              ...
              <p><strong>Invoice Date:</strong> {editableInvoice?.invoice_date}</p>
            )}
            ...
            {isEditMode ? (
              ...
              <p><strong>Total Amount:</strong> ₹ {editableInvoice?.TotalAmount}</p>
            )}
            ...
            <div>
              ...
              <ul>
                {editableInvoice && editableInvoice.ProfessionalFees.map((fee, index) => (
                  <li className="ml-5" key={index}>
                    {isEditMode ? (
                      <input
                        ...
                        onChange={(e) => {
                          const updatedFees = [...editableInvoice.ProfessionalFees];
                          updatedFees[index].description = e.target.value;
                          setEditableInvoice({ ...editableInvoice, ProfessionalFees: updatedFees });
                        }}
                        className="border p-2 w-full"
                      />
                    ) : (
            ...
            <div>
              ...
              <ul>
                {editableInvoice && editableInvoice.ReimbursementExpenses.map((expense, index) => (
                  <li className="ml-5" key={index}>
                    {isEditMode ? (
                      <input
                        type="text"
                        ...
                        onChange={(e) => {
                          const updatedExpenses = [...editableInvoice.ReimbursementExpenses];
                          updatedExpenses[index].description = e.target.value;
                          setEditableInvoice({ ...editableInvoice, ReimbursementExpenses: updatedExpenses });
                        }}
                        className="border p-2 w-full"
                      />
                    ) : (
            ...
            <hr className="border-primary my-4" />
            <p><strong>Status:</strong> {editableInvoice && editableInvoice.isPaid ? "Paid" : "Unpaid"}</p>
          </div>
    ...
    <label key={key} className="flex items-center cursor-pointer gap-2">
      ...
      <span
        className={`font-semibold transition-colors duration-300 ${
          visibleColumns[key as keyof typeof visibleColumns] ? "text-[#3c50e0]" : "text-[#b9b9b9]"
        }`}
      >
        {label}
      </span>
  ...
  {/* <div className="flex items-center justify-center h-full">
    <h2 className="text-xl font-semibold text-gray-800 dark:text-white">Invoice Filter</h2>
  </div> */}
  ...
  <div className="flex flex-col gap-1">
    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Paid Status</label>
    ...
    >
      ...
      <div className="bg-white border border-[#3c4fe01d] rounded-lg shadow-xl mt-6 overflow-hidden">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] text-center font-semibold text-black items-center  shadow-md dark:border-strokedark">
          ...
          {visibleColumns.invoiceid && <div className="border-r border-[#d6d6d6] p-3">Invoice No.</div>}
          {visibleColumns.company && <div className="border-r border-[#d6d6d6] p-3 flex ml-7">Company <FaSort  className="mt-1 ml-2 text-xl"/> 
 </div>}
          {visibleColumns.date && <div className="border-r border-[#d6d6d6] p-3">Date</div>}
          {visibleColumns.ProfessionalFees_total && <div className="border-r  border-[#d6d6d6] p-3">Prof. Fees</div>}
          {visibleColumns.ReimbursementExpenses_total && <div className="border-r border-[#d6d6d6] p-3">Reimb. Fees</div>}
          {visibleColumns.amount && <div className="border-r border-[#d6d6d6] p-3">Total Amount</div>}
          {visibleColumns.unpaidSection && (
            <div
              className={`border-r border-[#d6d6d6] p-3 ${
                editableInvoices[invoices[0]._id] ? "opacity-100" : "opacity-50 cursor-not-allowed"
              }`}
            >
              Paid
            </div>
...
{visibleColumns.unpaidSection && (
  ...
  >
    Pending R.
          ...
          {visibleColumns.due_amount && <div className="border-r border-[#d6d6d6] p-3">Due Amount</div>}
          {visibleColumns.action && <div className="p-2">Action</div>}
        </div>

              <div className="max-h-96 overflow-y-auto text-black hide-scrollbar" style={{
                                scrollbarWidth: "none", // Firefox
                                msOverflowStyle: "none", // Internet Explorer 10+
                                }}>
                        {filteredInvoices.slice().reverse().map((invoice) => (
                   <div key={invoice._id} className="grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] text-center items-center border-b border-stroke dark:border-strokedark">

                          {visibleColumns.invoiceid && <div className="p-2 border-[#ebebeb] border-r">{invoice.invoiceid}</div>}
                          {visibleColumns.company && <div className="p-2 border-[#ebebeb] border-r">{invoice.company?.companyName}</div>}
                          {visibleColumns.date && <div className="p-2 border-[#ebebeb] border-r">{formatDate(invoice.invoice_date)}</div>}
                          {visibleColumns.ProfessionalFees_total && <div className="p-2 bg-[#e6ccff3a] border-[#ebebeb] border-r">{invoice.ProfessionalFees_total}</div>}
                          {visibleColumns.ReimbursementExpenses_total && <div className="p-2 bg-[#ccfffb44] border-[#ebebeb] border-r">{invoice.ReimbursementExpenses_total}</div>}
                          {visibleColumns.amount && <div className="p-2 bg-[#ccffcf70] font-semibold border-[#ebebeb] border-r">{invoice.TotalAmount}</div>}
                            {visibleColumns.unpaidSection && (
                          ...
                          <input
                              ...
                              className={`${
                                editableInvoices[invoice._id] ? "opacity-100" : "opacity-50 cursor-not-allowed"
                              }`}

                            onChange={(e) => {
                              const isChecked = e.target.checked;
                              const updatedProfessionalFees = isChecked ? invoice.ProfessionalFees_total : 0;
                              const updatedReimbursementExpenses = isChecked ? invoice.ReimbursementExpenses_total : 0;
                              const updatedDueAmount =
                                Number(invoice.TotalAmount || 0) - (updatedProfessionalFees + updatedReimbursementExpenses);
                                ...
                                prevInvoices.map((inv) =>
                                  inv._id === invoice._id
                                    ? {
                                        ...inv,
                                        isPaid: isChecked,
                                        unpaid_ProfessionalFees: updatedProfessionalFees,
                                        unpaid_ReimbursementExpenses: updatedReimbursementExpenses,
                        ...
                        {visibleColumns.unpaidSection && (
                          ...
                          <input
                            ...
                            className={`w-24 px-2 text-center bg-[#ccfffb44] border-[#ebebeb] border-r ${
                              editableInvoices[invoice._id] ? "opacity-100" : "opacity-25 cursor-not-allowed"
                            }`}
                            ...
                            onChange={(e) => {
                              ...
                              Number(invoice.TotalAmount || 0) -
                                (newValue + (invoice.unpaid_ReimbursementExpenses || 0));

                              setInvoices((prevInvoices) =>
                                prevInvoices.map((inv) =>
                                  inv._id === invoice._id
                                    ? { ...inv, unpaid_ProfessionalFees: newValue, due_amount: updatedDueAmount }
                                    : inv
                                )
                              );
                            }}
                            onBlur={async (e) => {
                              const newValue = parseFloat(e.target.value) || 0;
                              try {
                                await axios.patch(`${baseUrl}/api/invoices/${invoice._id}`, {
                                  unpaid_ProfessionalFees: newValue,
                                });
                              } catch (error) {
                                console.error("Failed to update unpaid professional fees:", error);
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setInvoices((prevInvoices) =>
                                prevInvoices.map((inv) =>
                                  inv._id === invoice._id
                                    ? { ...inv, unpaid_ReimbursementExpenses: newValue, due_amount: updatedDueAmount }
                                    : inv
                                )
                              );
                            }}
                            onBlur={async (e) => {
                              ...
                              try {
                                ...
                                console.error("Failed to update unpaid reimbursement expenses:", error);
                      ...
                      {visibleColumns.due_amount && (
                          ...
                          className={`font-bold w-16 mx-auto rounded-xl ${
                            invoice.due_amount === 0
                              ...
                              : invoice.due_amount <= parseFloat(invoice.TotalAmount)
  ...
  <div className="flex items-center justify-center p-2">
    ...
    >
      {selectedInvoiceForDelete === invoice._id ? (
        ...
        <>
          {isDeleteModalOpen && selectedInvoiceForDelete === invoice._id && (
  <div className="absolute inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg text-black">
      <div className="text-sm text-center">
        <p className="tracking-wider">Are you sure you want to delete this invoice?</p>
